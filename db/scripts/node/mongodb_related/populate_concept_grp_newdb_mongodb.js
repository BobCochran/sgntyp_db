 ...
 * 'congrp' collection in the MongoDB database named 'sgtypdb2'. The     *
 * numeric field is indexed and we should get an error on duplicates.    *
 *-----------------------------------------------------------------------*
 * 2014-03-21 This script is specifically designed to insert documents to*
 * the collection. The revised, renamed FileNamesand CaptionKeys.txt     *
 * input file is a great deal larger in size than the original file. This*
 * collection is going to be indexed on the first field "cgrp_int_num" as*
 * a unique  index with duplicate dropping turned on.                    *
...
/* Database name sgtypdb2, journalling is on, and an index will be manually applied through the mongo
 * shell. One can search the .dbshell file to discover the exact indexing applied.
 */
...
/*
 *This script is updating a collection which is kept on a separate physical host. The database does not
 * live on the localhost. It takes time for all the database inserts to complete. Set a timer to allow time
 * for the complete event loop to finish. This script starts out with 60 seconds. If that proves insufficient,
 * we will double it to 120 seconds. If that is not enough, then we will treble it to 180 seconds.
 *
 */
